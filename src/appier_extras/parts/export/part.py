...
class ExportPart(appier.Part):
    ...
    def routes(self):
        return [
            ...
            (
                ("GET",),
                "/export/models/<str:model>/<str:_id>.json",
                self.entity_json,
                None,
                True,
            ),
            (
                ("POST",),
                "/api/export/models/<str:model>",
                self.model_import_api,
                None,
                True,
            ),
        ]
    ...
    def model_json(self, model):
        ...
        models = self.owner.admin_part._find_view(
            model_c, map=True, rules=False, **object
        )
        return models
    ...
    def json(self):
        return appier.get_app().url_for(
            "export.entity_json", model=self.__class__._under(), _id=self._id
        )

    def _monkey_patch(
        self, models=None, base_cls=None, class_methods=None, instance_methods=None
    ):
        ...
        instance_methods = instance_methods or (ExportPart.json,)
        ...
        for model in models:
            ...
            if not issubclass(model, base_cls):
                continue
            ...
            if "_methods" in model.__dict__:
                del model.__dict__["_methods"]
