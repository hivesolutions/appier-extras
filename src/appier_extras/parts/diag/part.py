...
class DiagPart(appier.Part):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.max_data_size = appier.conf(
            "DIAG_MAX_DATA_SIZE", self.max_data_size, cast=int
        )
    ...
    def load(self):
        ...
        if self.owner.admin_part:
            self.owner.admin_part.add_section_item(
                "Requests", "diag.list_requests", section="Diag"
            )
        ...
        if self.empty:
            models.DiagRequest.delete_c()

    def unload(self):
        appier.Part.unload(self)

        if self.owner.admin_part:
            self.owner.admin_part.remove_section_item("Requests", section="Diag")
    ...
    def after_request(self):
        try:
            ...
            self.owner.log_error(exception, message="Problem running diag logging: %s")
    ...
    def list_requests(self):
        object = appier.get_object(alias=True, page=True, find=True, sort=(("id", -1),))
    ...
    def _store_log(self):
        ...
        data = (
            self.request.data
            if self.request.data and len(self.request.data) < self.max_data_size
            else None
        )
    ...
    def _get_loggly_api(self):
        ...
        try:
            loggly = appier.import_pip("loggly", package="loggly_api_python")
        except Exception:
            loggly = None
        if not loggly:
            return None
    ...
    def _get_item_debug(self):
        ...
        item.update(info_dict=self.owner.info_dict())
        return item
