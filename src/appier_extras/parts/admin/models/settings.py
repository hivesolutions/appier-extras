...
class Settings(base.Base):
    facebook_token = appier.field(index="hashed")

    github_token = appier.field(index="hashed")

    google_token = appier.field(index="hashed")

    google_refresh_token = appier.field(index="hashed")

    google_email = appier.field(index="hashed")

    google_scope = appier.field(type=list)

    live_token = appier.field(index="hashed")

    twitter_token = appier.field(index="hashed")

    twitter_token_secret = appier.field(index="hashed")

    extra = appier.field(type=dict)
    ...
    def linked_apis(cls):
        ...
        if settings.facebook_token:
            linked["facebook"] = settings.facebook_token
        if settings.github_token:
            linked["github"] = settings.github_token
        if settings.google_token:
            linked["google"] = settings.google_email
        if settings.live_token:
            linked["live"] = settings.live_token
        if settings.twitter_token:
            linked["twitter"] = settings.twitter_token
        return linked
