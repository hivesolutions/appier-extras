...
class Google(object):
    ...
    def has_google(self):
        ...
        if not google:
            return False
        if not appier.conf("GOOGLE_ID"):
            return False
        if not appier.conf("GOOGLE_SECRET"):
            return False
        return True

    def ensure_google_account(self, create=True, safe=True, next=None):
        api = self.get_google_api()
        user = api.self_user()
        email = user["emails"][0]["value"]
        google_id = user["id"]
        account = self.owner.admin_account.get(
            google_id=google_id, rules=False, raise_e=False
        )
    ...
    def ensure_google_api(
        ...
        return api.oauth_authorize(
            state=state, access_type=access_type, approval_prompt=approval_prompt
        )
