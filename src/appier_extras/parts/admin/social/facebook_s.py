...
class Facebook(object):
    ...
    def has_facebook(self):
        ...
        if not facebook:
            return False
        if not appier.conf("FB_ID"):
            return False
        if not appier.conf("FB_SECRET"):
            return False
        return True
    ...
    def ensure_facebook_account(self, create=True, safe=True, next=None):
        ...
        account = self.owner.admin_account.get(
            facebook_id=facebook_id, rules=False, raise_e=False
        )
        account = account or self.owner.admin_account.from_session()
        account = (
            account
            or email
            and self.owner.admin_account.get(email=email, rules=False, raise_e=False)
        )
