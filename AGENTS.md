...
To run the custom suite of unit test for Appier Extras use the following sequence of commands that will install dependencies
...
- Increment (look at `CHANGELOG.md` for semver changes) the `version` value in `setup.py`, and then the `VERSION` value in `src/appier_extras/base.py`.
...
Hive Appier Framework is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
